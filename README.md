...
权重默认从 **ModelScope**（国内直连）自动下载，HuggingFace 兜底，无需手动干预。
缓存位置：`%USERPROFILE%\.cache\modelscope`（可用 `MODELSCOPE_CACHE` 改）。

| 命令 | 说明 |
|---|---|
| `assistlabel models list` | 浏览所有可用模型（含中文说明） |
| `assistlabel models info KEY` | 查看单个模型规格 |
| `assistlabel models download KEY` | 预下载权重 |
...
| key | 说明 | 许可 |
|---|---|---|
| `sam3` | SAM3 检测+实例分割（默认检测引擎） | Meta 自定义 |
| `da3-metric-L` | DA3 米制深度 0.35B（**推荐默认**） | Apache-2.0 |
| `da3-mono-L` | DA3 相对深度 0.35B | Apache-2.0 |
| `da2-metric-indoor-L/B` | DA2 室内米制深度 | Apache-2.0 |
| `da2-metric-outdoor-L/B` | DA2 室外米制深度 | Apache-2.0 |
| `da2-relative-L` | DA2 相对深度 | Apache-2.0 |
...
```yaml
dataset:
  image_dir: "data/raw"          # 源图像目录（递归扫描）
  out_dir: "labeled"             # 输出根目录
  patterns: ["*.jpg", "*.png"]   # 图像文件匹配模式

tasks: [depth, detect]           # 要执行的任务（任意子集）

download:
  source: auto                   # auto = ModelScope 优先, HF 兜底

depth:
  model: "da3-metric-L"          # 深度引擎（见 models list）
  device: cuda
  half: true                     # fp16 推理
  batch_size: 8                  # GPU 批量；32G 显存建议 8~16，OOM 减半
  prefetch: 4                    # 预取解码线程数
  write_workers: 2               # 写盘线程池

detect:
  model: "sam3"                  # 检测引擎
  ontology: "ontology.yaml"      # 类别定义
  conf_thres: 0.5                # 置信度阈值
  nms_iou: 0.7                   # 类内 NMS
  viz: true                      # 生成 detect_viz 叠加图
```
